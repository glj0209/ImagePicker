...
//    }
    cell.enebleView.hidden = YES;
    if (self.imagePickerController.selectedCellArray.count >= self.imagePickerController.maximumNumberOfSelection) {
        cell.enebleView.hidden = NO;
    }
    
    for (int i = 0; i < self.imagePickerController.selectedCellArray.count; i++) {
        NSDictionary *dic = self.imagePickerController.selectedCellArray[i];
        if ([dic[@"cameraName"] isEqualToString:self.navigationItem.title]) {
            if (dic[@"iamgeindex"] == indexPath) {
                NSInteger index = [dic[@"index"] integerValue];
                cell.nuLabel.text = [NSString stringWithFormat:@"%ld",(long)index];
                
                NSDictionary *tmpdic = @{@"cell":cell,@"iamgeindex":indexPath,@"cameraName":self.navigationItem.title,@"index":@(index)};
                [self.imagePickerController.selectedCellArray removeObjectAtIndex:i];
                [self.imagePickerController.selectedCellArray insertObject:tmpdic atIndex:i];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (imagePickerController.allowsMultipleSelection) {
        ...
        cell.nuLabel.text = [NSString stringWithFormat:@"%lu",(unsigned long)selectedAssets.count];
        
        // 记录选中的cell位置
        NSInteger index = selectedAssets.count;
        NSDictionary *dic = @{@"cell":cell,@"iamgeindex":indexPath,@"cameraName":self.navigationItem.title,@"index":@(index)};
        [self.imagePickerController.selectedCellArray addObject:dic];
    ...
    if (selectedAssets.count >= self.imagePickerController.maximumNumberOfSelection) {
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    QBAssetCell *tmpCell = (QBAssetCell *)[collectionView cellForItemAtIndexPath:indexPath];
    BOOL edit = NO;
    for (int i = 0; i < self.imagePickerController.selectedCellArray.count; i++) {
        NSDictionary *dic = self.imagePickerController.selectedCellArray[i];
        if ([dic[@"cameraName"] isEqualToString:self.navigationItem.title]) {
            if (dic[@"cell"] == tmpCell || dic[@"iamgeindex"] == indexPath) {
                edit = YES;
            }else {
                if (edit) {
                    
                    NSInteger index = [dic[@"index"] integerValue];
                    QBAssetCell *cell = (QBAssetCell *)dic[@"cell"];
                    
                    index -= 1;
                    cell.nuLabel.text = [NSString stringWithFormat:@"%ld",(long)index];
                    
                    NSDictionary *tmpDic = @{@"cell":cell,@"iamgeindex":dic[@"iamgeindex"],@"cameraName":dic[@"cameraName"],@"index":@(index)};
    ...
    [self.imagePickerController.selectedCellArray removeAllObjects];
    [self.imagePickerController.selectedCellArray addObjectsFromArray:tmpArray];
    ...
    if ((selectedAssets.count + 1) >= self.imagePickerController.maximumNumberOfSelection) {
        [collectionView reloadData];
    }
}
